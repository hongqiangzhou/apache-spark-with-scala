...
object DegreesOfSeparation {
  ...
  // A BFSNode has a heroID and the BSFData associated with it.
  type BFSNode = (Int, BFSData)

  // Converts a line of raw input into a BSFNode
  def convertToBFS(line: String): BFSNode = {
    ...
    // Unlesss this is the character we are starting from
  ...
  def bfsMap(node: BFSNode): Array[BFSNode] = {
    ...
    if (color == "GRAY") {
      for (connection <- connections) {
        ...
        // Have we stumbled across the character we are looking for? If so increment our accumulator so the driver script knows.
  ...
  def main(args: Array[String]): Unit = {
    ...
    for (iteration <- 1 to 10) {
      ...
      // Create new vertices as needed to darken or reduce distances in the reduce stage. If we encounter the node we are
      // looking for as a GRAY node, increment our accumulator to signal that we are done.
      val mapped = iterationRdd.flatMap(bfsMap)

      // Note that mapped.count() action here forces the RDD to be evaluated, and that is the only reason our accumulator is
      // actually updated.
