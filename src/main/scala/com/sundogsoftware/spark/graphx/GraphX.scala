package com.sundogsoftware.spark.graphx
...
object GraphX {
  ...
  def parseName(line: String): Option[(VertexId, String)] = {
  ...
  def makeEdges(line: String): List[Edge[Int]] = {
    ...
    for (x <- 1 to (fields.length - 1)) {
      // Our attribute field is unused, but in other graphs could be used to deep track of physical distances etc.
  ...
  def main(args: Array[String]): Unit = {
    ...
    val verts = names.flatMap(parseName)
    ...
    val bfs = initialGraph.pregel(Double.PositiveInfinity, 10)(
      // Our "vertex program" preserves the shortest distance between an inbound message and its current value
      // It receives the vertex ID we are operating on, the attribute already stored with the vertex, and the inbound message
      // from this iteration.
      (id, attr, msg) => math.min(attr, msg),

      // Our "send message" function propagates out to all neighbors with the distance incremented by one
      triplet => {
      ...
      // The "reduce" operation preserves the minimum of messages received by a vertex if multiple messages are received by one vertex
      (a, b) => math.min(a, b)
    ).cache()
    ...
    // Recreate our "degree of separation" result:
